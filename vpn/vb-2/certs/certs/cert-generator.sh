...
openssl x509 -req -days 1825 -in "$NAME.csr" -CA ca.pem -CAkey ca-priv-key.pem -CAcreateserial -out "$NAME-cert.pem" -extensions v3_req -extfile <(echo "$CONFIG") > /dev/null;
